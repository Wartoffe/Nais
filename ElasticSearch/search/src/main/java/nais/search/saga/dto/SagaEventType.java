 ...
 *
 * Flow:
 *  - ColumnarDBService.createLoan drops availableCopies to 0
 *      -> publishes HIDE_REQUESTED
 *  - ColumnarDBService.returnBook raises availableCopies from 0
 *      -> publishes UNHIDE_REQUESTED
 *  - This service consumes *_REQUESTED, performs the logical
 *    delete/undelete locally, and publishes back *_COMPLETED on
 *    success or *_FAILED on failure.
 *  - ColumnarDBService consumes *_FAILED and compensates (rolls
 *    back) the original createLoan/returnBook transaction.
 */
